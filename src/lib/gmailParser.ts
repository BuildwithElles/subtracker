...
const TRIAL_KEYWORDS = [
  'free trial', 'trial period', 'trial expires', 'trial ends',
  'trial ending', 'trial subscription', 'start your trial',
  'your trial', 'trial version', 'premium trial'
]

const BILLING_KEYWORDS = [
  'subscription', 'billing', 'payment', 'charge', 'invoice',
  'receipt', 'renewal', 'auto-renewal', 'recurring'
]
...
export class GmailSubscriptionParser {
  ...
  parseEmail(email: EmailMessage): ParsedTrialEmail | null {
    ...
    const hasTrialKeywords = TRIAL_KEYWORDS.some(keyword => 
   ...
   * Extract pricing information
   */
  private extractPricing(content: string, fullBody: string): { amount?: number, currency?: string } {
   ...
   */
  private calculateConfidence(_email: EmailMessage, factors: {
